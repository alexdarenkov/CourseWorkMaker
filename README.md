...
- **Нормоконтроль**: проверка оформления по ГОСТ одной кнопкой (обязательные разделы,
  подписи таблиц/рисунков, ссылки на источники `[N]`, заголовки) — без ИИ и бесплатно
...
- Титульный лист из свободных блоков: подходит и для курсовой по ГОСТ, и для
  ...
  произвольная шапка
- **Свой титульник файлом**: загрузите готовый PDF/DOCX (например, бланк
  кафедры) — его первая страница станет титульным листом в превью и в экспорте
...
- **Экспорт в PDF** — сервер дорендеривает DOCX через LibreOffice: содержание
  заполняется реальными номерами страниц, формулы сохраняются
...
- **Отмена без потерь**: при остановке генерации уже написанные разделы можно
  забрать в редактор (показываются в diff-просмотре)
- **Правка одного раздела**: выбор раздела во вкладке «Исправить» — ИИ переписывает
  только его (быстрее и дешевле), остальной текст не тронется
- **Diff-просмотр правки**: перед применением видно, что добавится и удалится, —
  правку можно отклонить одной кнопкой
- **Откат ИИ-правки**: текст до применения результата ИИ сохраняется — кнопка
  «Откатить» в статус-баре возвращает его (повторное нажатие — обратно)
- **Проверка источников**: нормоконтроль проверяет доступность URL из списка
  литературы (ИИ иногда придумывает ссылки)
...
| `POST` | `/api/convert/pdf` | то же тело → файл .pdf (LibreOffice, содержание заполнено) |
| `POST` | `/api/convert/title-image` | multipart `file` (PDF/DOCX) → `{image}` — первая страница титульника как PNG |
...
| `POST` | `/api/ai/edit-section` | `{instruction, section_title, markdown}` → `{jobId}` — правка одного раздела |
...
| `POST` | `/api/ai/lint` | `{markdown, check_urls?}` → `{issues}` — нормоконтроль без LLM (работает без `AI_API_KEY`); `check_urls` проверяет ссылки источников |
