...
- Титульный лист с настраиваемыми полями
...
| `POST` | `/api/ai/edit` | `{instruction, markdown}` → `{jobId}` — правка документа |
| `GET` | `/api/ai/jobs/{id}` | Статус/прогресс/результат генерации |
...
Кнопка «Загрузить .md» в тулбаре открывает свой Markdown-файл в редакторе:
- `http(s)`/`data:`-картинки работают сразу
- Для локальных ссылок предлагается приложить файлы (сопоставляются по имени)
- Неприложенные изображения становятся заглушками
...
| Подписи рисунков по центру под иллюстрацией | ✅ |
| Подписи таблиц слева над таблицей | ✅ |
| Формулы по центру с номером справа | ✅ |
